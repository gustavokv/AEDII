...
//Função para rotacionar à esquerda, diferente da caso2 que também rotaciona, porém esta é utilizada somente
//para a remoção.
struct s_no *leftRotate(struct s_no *x){
    struct s_no *y = x->dir;
...
//Função para rotacionar à direita, diferente da caso1 que também rotaciona, porém esta é utilizada somente
//para a remoção.
struct s_no *rightRotate(struct s_no *y){
    struct s_no *x = y->esq;
